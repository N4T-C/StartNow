...
const makeCacheKey = (mode, idea, refinement) => {
  const raw = `${mode}|${idea.toLowerCase()}|${(refinement || "").toLowerCase()}`;
...
const analyzeController = async (req, res) => {
  try {
    ...
    const allRegions = buildRegionSignals(idea, mode, refinement || undefined);
    const topRegions = allRegions.slice(0, 5);
    const summary = summarizeSignals(allRegions);

    const { insight, providerUsed } = await generateInsights({ mode, idea, refinement, topRegions });
