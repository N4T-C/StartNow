...
## Business Mode Credentials

| Email | Password | Company | Role |
|---|---|---|---|
| hclmanager@hcl.in | hclman | HCL Technologies | Manager |
| tatadirector@tata.in | tataman | Tata Group | Director |
| reliancehead@reliance.in | reliman | Reliance Industries | Head |
| infosyspm@infosys.in | infoman | Infosys | Product Manager |
| wiproceo@wipro.in | wiproman | Wipro | CEO |

## Authentication

### Google OAuth (Startup Mode)
- On first load, users are prompted to sign in with Google via Firebase Auth.
- After sign-in, the full map dashboard is shown.
- Auth state persists across page refreshes automatically.

### Business Mode
- Clicking the **Business** toggle opens a credential modal instead of switching immediately.
- Enter your company email and password (see table above).
- On success, the app switches to Business mode and re-runs analysis if an idea is already set.
- Credentials are verified against MongoDB via `POST /api/auth/business-login`.
